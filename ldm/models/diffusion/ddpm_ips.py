...
class DDPM(pl.LightningModule):
    ...
    def register_schedule(self, given_betas=None, beta_schedule="linear", timesteps=1000,linear_start=1e-4, linear_end=2e-2, cosine_s=8e-3):
        ...
        #predict_start_from_noise #Get the distribution p(x_0 | x_t). 
    ...
    def predict_start_from_z_and_v(self, x_t, t, v):
        #Get the distribution p(x_0 | z,v).
        return (
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        #self.instantiate_cond_stage(cond_stage_config)#ldm.modules.encoders.modules.FrozenCLIPEmbedder
    ...
    def register_schedule(self,
        ...
        self.shorten_cond_schedule = self.num_timesteps_cond > 1#1>1 
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if self.model.conditioning_key is not None and not self.force_null_conditioning:#执行了这段话
            if cond_key is None:
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        x_recon = self.model(x_noisy, t, **cond)#unet 
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        model_output = self.apply_model(x_noisy, t, cond)#unit 
    ...
    def progressive_denoising(self, cond, shape, verbose=True, callback=None, quantize_denoised=False,
        ...
        for i in iterator:#倒序
            ts = torch.full((b,), i, device=self.device, dtype=torch.long)#batch 
            if self.shorten_cond_schedule:
                assert self.model.conditioning_key != 'hybrid'
                tc = self.cond_ids[ts].to(cond.device)
                #获取xt的，平常sample 不需要梯度，但是这个要梯度
    ...
    def p_sample_loop(self, cond, shape, return_intermediates=False,
        ...
        if not log_every_t:
            log_every_t = self.log_every_t#100 DDPM 定义的
        device = self.betas.device#register schedule里面定义了这个了
        ...
        for i in iterator:# 
            ts = torch.full((b,), i, device=device, dtype=torch.long)#999不然的话。。
            if self.shorten_cond_schedule:#值为false 基本不用看了，
                assert self.model.conditioning_key != 'hybrid'
                tc = self.cond_ids[ts].to(cond.device)
                cond = self.q_sample(x_start=cond, t=tc, noise=torch.randn_like(cond))
...
class Fin_Aug_diffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.concat_mode = concat_mode#True 
        self.cond_stage_trainable = cond_stage_trainable#False，给定的也是false
        self.cond_stage_key = cond_stage_key #"caption" 了变成就是说到这里是latent diffusion 吗，感觉
        try:
            self.num_downs = len(first_stage_config.params.ddconfig.ch_mult) - 1#2-1=1
        except:
            self.num_downs = 0
    ...
    #5这个cond_stage_model 在2 里刚定义完
    def get_learned_conditioning(self, c):
        if self.cond_stage_forward is None:#确实是none
            if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                c = self.cond_stage_model.encode(c)
                if isinstance(c, DiagonalGaussianDistribution):
                    c = c.mode()#return self.mean
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:#latent diffustion 里面被设置成了crossattn 
            assert c is not None# 既然你使用了conditioning_key 就得有condition吧
            if self.cond_stage_trainable:#False  这个位置就相当于直接的使用了c的信息
                c = self.get_learned_conditioning(c)
            if self.shorten_cond_schedule:  # TODO: drop this option
                tc = self.cond_ids[t].to(self.device)
                c = self.q_sample(x_start=c, t=tc, noise=torch.randn_like(c.float()))
    ...
    def _predict_eps_from_xstart(self, x_t, t, pred_xstart):
        ...
        return (extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x_t.shape) * x_t - pred_xstart) / \
               extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, x_t.shape)
    #17 其给定了最后一个时刻，的noise 到正泰分布还举例多远
    # num_timesteps越大越是正态分布了
    def _prior_bpd(self, x_start):
        """
        Get the prior KL term for the variational lower-bound, measured in
        bits-per-dim.
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        if self.parameterization == "x0":
            ...
            raise NotImplementedError()
        loss_simple = self.get_loss(model_output, target, mean=False).mean([1, 2, 3])#直接就这么计算了，
        loss_dict.update({f'{prefix}/loss_simple': loss_simple.mean()})
...
#         print("start loss observation !")
        logvar_t = self.logvar[t].to(self.device)#
        loss = loss_simple / torch.exp(logvar_t) + logvar_t#大家不同时刻，有不同的状态，分别计算一下
        # loss = loss_simple / torch.exp(self.logvar) + self.logvar
        if self.learn_logvar:
            loss_dict.update({f'{prefix}/loss_gamma': loss.mean()})
            loss_dict.update({'logvar': self.logvar.data.mean()})
...
#         loss_dict.update({f'{prefix}/loss': loss})
        ...
        abs_original =torch.mean(torch.abs(x_start)) 
        abs_train =torch.mean(torch.abs(x_recon))
        abs_loss= torch.abs(abs_original-abs_train)  
        loss += 0.3*abs_loss
...
#         y=asd+3
    ...
    def p_mean_variance(self, x, c, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
        ...
        if return_codebook_ids:
            ...
            return model_mean, posterior_variance, posterior_log_variance
    #20
    #在获得了前一个时刻的分布之后#q(xt-1|xt,x0)
    #就使用均值和方差进行了采样，中间code book x0的东西有点多
    #采样没梯度，
    @torch.no_grad()
    def p_sample(self, x, c, t, clip_denoised=False, repeat_noise=False,
                 return_codebook_ids=False, quantize_denoised=False, return_x0=False,
    ...
    def progressive_denoising(self, cond, shape, verbose=True, callback=None, quantize_denoised=False,
        ...
        if start_T is not None:
            timesteps = min(timesteps, start_T)#还可能从中间的时刻进行读取。
        #verbose 是true
        iterator = tqdm(reversed(range(0, timesteps)), desc='Progressive Generation',
                        total=timesteps) if verbose else reversed(range(0, timesteps))
        if type(temperature) == float:
        ...
        for i in iterator:
            ts = torch.full((b,), i, device=self.device, dtype=torch.long)#batch 这么个长度的t
            if self.shorten_cond_schedule:
                assert self.model.conditioning_key != 'hybrid'
                tc = self.cond_ids[ts].to(cond.device)
    ...
    def p_sample_loop(self, cond, shape, return_intermediates=False,
        ...
        for i in iterator:# 
            ...
            if self.shorten_cond_schedule:#值为false ，
                assert self.model.conditioning_key != 'hybrid'
                tc = self.cond_ids[ts].to(cond.device)
                cond = self.q_sample(x_start=cond, t=tc, noise=torch.randn_like(cond))
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        x_recon = self.model(x_noisy, t, **cond)#unet 的向前
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        #
#         print("start loss observation !")
        logvar_t = self.logvar[t].to(self.device)#这
        loss = loss_simple / torch.exp(logvar_t) + logvar_t#
        # loss = loss_simple / torch.exp(self.logvar) + self.logvar
        if self.learn_logvar:
            loss_dict.update({f'{prefix}/loss_gamma': loss.mean()})
